...
import com.equipmentverleih.exceptions.*;
 ...
 * Tätigkeiten durchzuführen sind. <code>
...
public final class LdapUser {
	 ...
	 *                 gecachet werden un damit ausgelesen werden könnten. Bitte
	 *                 achten sie auch darauf, dass sie in der restlichen Anwendung
	 *                 Passwörter nie als Strings speichern.
	 *
	...
	public LdapUser(String userId, char[] password) throws LdapException, LdapAuthException {
		...
		try {
			...
			NamingEnumeration<SearchResult> renum = context.search(baseDN, "cn=" + userId, controls);
			...
			// User, Group, etc...
			// Schüler: distinguishedName:
			// CN=it150178,OU=4AHITM,OU=IT,OU=Students,OU=HTL,DC=EDU,DC=HTL-LEONDING,DC=AC,DC=AT
			// Lehrer: distinguishedName:
			// CN=m.widmann,OU=Teachers,OU=HTL,DC=EDU,DC=HTL-LEONDING,DC=AC,DC=AT
			...
			if (distinguishedname[1].contains("Teacher")) {
				...
				for (String attribute : attributes) {
					if (attribute.startsWith("OU")) {
						...
						if (keyValue[1].charAt(0) - (char) '0' >= 0 && keyValue[1].charAt(0) - (char) '0' <= 9) {
